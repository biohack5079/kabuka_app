...
def delete_post(post_id):
    ...
    with open(bbs_file, 'w', encoding='utf-8') as f:  # Open the file in write mode
        ...
        for line in lines:  # Iterate through each line
            if f'id="{post_id}"' in line:  # Check if the line contains the post ID
...
if action == "Delete" and post_id:
    delete_post(post_id)  # Delete the post if action is "Delete"

if action == "Report" and post_id:
    report_post(post_id)  # Report the post if action is "Report"
